...
about API usage. Note, the command-line binary ``neato`` from ``graphviz``
as well as the ``graphviz`` Python package are required for making the unused
and used API entry graphs.
