...
the example gallery. A list of related examples is automatically generated by
computing the closest examples in the TF-IDF space of their text contents. Then,
the most similar content is displayed at the bottom of each example as a set of
thumbnails.
...
The parameters ``min_df`` and ``max_df`` can be custom by the user to trim the
