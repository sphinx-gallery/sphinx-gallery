...
* **org** is the GitHub organization where your documentation is stored.
* **repo** is the GitHub repository where your documentation is stored.
* **ref** is a reference to the version of your repository where your documentation exists.
  For example, if your built documentation is stored on a ``gh-pages`` branch, then this field
  should be set to ``gh-pages``.
* **binderhub_url** is the full URL to a BinderHub deployment where you want your examples to run. One
...
* **dependencies** is a list of paths (relative to ``conf.py``) to dependency files that Binder uses to infer the environment needed
...
* **filepath_prefix** (default: ``None``) is a prefix to append to the filepath in the Binder links. You should use this if you will store your built
  documentation in a sub-folder of a repository, instead of in the root.
* **notebooks_dir** (default: ``notebooks``) is the name of a folder where the built Jupyter notebooks will be copied. This ensures that all the notebooks are
...
* **use_jupyter_lab** (default: ``False``) is a boolean that determines whether the default interface activated by Binder will be for
