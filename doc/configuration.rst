...
- ``components_pattern`` (:ref:`components_pattern`)
...
- ``sg_download_links``: The download links of source code, Jupyter notebook, and the
  zip of them for an example. Note that if a download link does not exist on the example
  page, the link will not be generated by this component either.
- ``sg_launcher_links``: The launcher links for JupyterLite and Binder for an example.
  Note that if a launcher link does not exist on the example page (e.g., one does not
  activate that launcher), the link will not be generated by this component either.

The ``components_pattern`` configuration allows you to specify a list of glob patterns
to match the pages that should include these components (by default, ``[]``).

In order to hide the original download and launcher links on the page, one can use
``display: none`` in CSS, see :ref:`hide_download_buttons-note`. The relevant classes
are:
...
Restrict which pages the Sphinx-Gallery components should be applied to:

.. code-block:: python

    sphinx_gallery_conf = {
        ...
        "components_pattern": ["auto_examples/*"],
    }
...
If one wants to add the CSS only for certain pages (e.g., one wants to hide the links
only for one of multiple galleries or one wants to hide different links for different
galleries), one can do the following:
