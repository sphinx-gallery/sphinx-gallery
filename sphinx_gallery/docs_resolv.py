...
def _get_data(url):
    ...
    if urllib_parse.urlparse(url).scheme in ('http', 'https'):
        ...
        if encoding == 'gzip':
            ...
            raise ExtensionError('unknown encoding {!r}'.format(encoding))
...
def _handle_http_url_error(e, msg='fetching'):
    if isinstance(e, HTTPError):
        error_msg = '{} {}: {} ({})'.format(msg, e.url, e.code, e.msg)
    elif isinstance(e, URLError):
        error_msg = '{}: {}'.format(msg, e.reason)
            ...
            for cobj in cobjs:
                for modname in (cobj['module_short'], cobj['module']):
                    ...
                    if this_module in doc_resolvers:
                        try:
                            ...
                            _handle_http_url_error(
                                e, msg='resolving {}.{}'.format(modname, cname))
                    ...
                    if link is not None:
                        ...
                        full_function_name = '{}.{}'.format(modname, cname)
